...
REGION_CODE, REGION_NAME, CATEGORY_NAME = 'jangheung', '장흥군', '전남'
BASE_URL = 'https://www.jangheung.go.kr'
LIST_URL = 'https://www.jangheung.go.kr/www/news/press'
        ...
        for pn in range(1,4):
            ...
            for i in range(min(rows.count(),10)):
                try:
                    ...
                    r = send_article_to_server({'title':title,'content':content or url,'published_at':f"{datetime.now().strftime('%Y-%m-%d')}T09:00:00+09:00",'original_link':url,'source':REGION_NAME,'category':CATEGORY_NAME,'region':REGION_CODE,'thumbnail_url':thumb})
                    collected += 1
                    if r.get('status')=='created': success += 1
                    safe_goto(page, f'{LIST_URL}?page={pn}')
                except: continue
        browser.close()
    log_to_server(REGION_CODE, '성공', f'완료', 'success')
    print(f"✅ 완료 ({collected}개)")

def main():
    import argparse
    collect_articles(argparse.ArgumentParser().add_argument('--days',type=int,default=3) or 3)
