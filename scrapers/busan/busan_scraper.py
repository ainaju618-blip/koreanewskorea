"""
부산광역시 보도자료 스크래퍼 - Enterprise Stealth v3.0
...
LIST_URL = 'https://www.busan.go.kr/nbtnewsBU'

# 페이지네이션 패턴: ?curPage={page_num}
PAGE_PARAM = 'curPage'

# 목록 페이지 셀렉터 (부산시 카드 레이아웃)
LIST_ROW_SELECTORS = [
    'main ul > li',
    '.board_list ul > li',
    'ul.list > li',
...
def fetch_detail(page: Page, url: str, delay_gen: PODMPDelayGenerator) -> Tuple[str, Optional[str], str, Optional[str]]:
    ...
    # 3. 이미지 추출
    # 전략 1: 부산시 첨부파일 링크 (a[href*='/comm/getFile'])
    attach_links = page.locator('a[href*="/comm/getFile"], a[href*="download"], a[href*="file"], a[href*="attach"]')
...
def collect_articles(days: int = 3, max_articles: int = 30, start_date: str = None, end_date: str = None, headless: bool = True) -> List[Dict]:
    ...
    print(f"   [INFO] 카드 레이아웃 모드 (ul > li 구조)")
    ...
    with sync_playwright() as p:
        ...
        while page_num <= 5 and not stop and collected_count < max_articles:
            ...
            print(f"      [FOUND] {row_count}개 항목 발견")
            ...
            for i in range(row_count):
                ...
                try:
                    ...
                    card_thumbnail = None
                    try:
                        img_elem = row.locator('img').first
                        if img_elem.count() > 0:
                            card_thumbnail = safe_get_attr(img_elem, 'src')
                            if card_thumbnail and not card_thumbnail.startswith('http'):
                                card_thumbnail = urljoin(BASE_URL, card_thumbnail)
                    except:
                        pass
            ...
            for item in new_link_data:
                ...
                # 카드 썸네일 폴백
                if not thumbnail_url and item.get('card_thumbnail'):
                    cloudinary_url = download_and_upload_image(item['card_thumbnail'], BASE_URL, folder=REGION_CODE)
                    if cloudinary_url:
                        thumbnail_url = cloudinary_url
                        error_reason = None
