...
def fetch_detail(page: Page, url: str) -> Tuple[str, Optional[str], str, Optional[str], Optional[str]]:
    ...
    # 이미지가 없으면 스킵
    if not thumbnail_url:
        return "", None, pub_date, None, ErrorCollector.IMAGE_MISSING
    
    return content, thumbnail_url, pub_date, None, None  # success
...
def collect_articles(days: int = 3, max_articles: int = 30, start_date: str = None, end_date: str = None) -> List[Dict]:
    ...
    with sync_playwright() as p:
        ...
        while page_num <= 5 and not stop and collected_count < max_articles:
            ...
            # Collect and send detail pages (only new articles)
