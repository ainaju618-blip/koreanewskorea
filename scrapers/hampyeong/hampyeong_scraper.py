...
REGION_CODE, REGION_NAME, CATEGORY_NAME = 'hampyeong', '함평군', '전남'
BASE_URL = 'https://www.hampyeong.go.kr'
# 실제 보도자료 URL (사용자 브라우저에서 확인)
LIST_URL = 'https://www.hampyeong.go.kr/boardList.do?pageId=www275&boardId=NEWS'
LIST_SELECTORS = ['a[href*="boardView"]', 'a[href*="view"]', 'tbody tr td.title a', '.board_list a']
CONTENT_SELECTORS = ['div.view_content', 'div.board_view', 'div.board_view_body', '.cont_view', '.view_cont', '#txt', '.con_txt']
...
def collect_articles(days=3):
    ...
    with sync_playwright() as p:
        ...
        for pn in range(1,4):
            list_url = f'{LIST_URL}&page={pn}'
            if not safe_goto(page, list_url): continue
            links = wait_and_find(page, LIST_SELECTORS, timeout=10000)
            if not links: break
            print(f"   📄 페이지 {pn}: {links.count()}개")
            for i in range(min(links.count(),10)):
                try:
                    ...
                    if 'view' not in href.lower() and 'View' not in href: continue
        ...
        browser.close()
    log_to_server(REGION_CODE, '성공', f'완료 ({collected}개)', 'success')
