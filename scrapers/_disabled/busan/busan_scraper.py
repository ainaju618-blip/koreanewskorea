# -*- coding: utf-8 -*-
"""부산광역시 보도자료 스크래퍼 v4.0 (Simple Playwright)
- 운영서버 방식으로 단순화
- 카드 레이아웃 (ul > li 구조)
"""
...
LIST_ROW_SELECTORS = [
    'main ul > li',
    'ul > li:has(a[href*="/nbtnewsBU/"])',
    '.board_list li',
]

CONTENT_SELECTORS = [
    'div.boardView',  # 부산시 본문 영역
    'div.view_content',
    'div.board_view',
    '.article_content',
    'div.bbs_view',
...
def fetch_detail(page: Page, url: str) -> Tuple[str, Optional[str], Optional[str], Optional[str]]:
    ...
    # 이미지 추출 - 첨부파일 링크
    attach_links = page.locator('a[href*="/comm/getFile"], a[href*="download"], a[href*="file"]')
...
def collect_articles(days: int = 3, max_articles: int = 30, start_date: str = None, end_date: str = None, headless: bool = True) -> List[Dict]:
    ...
    with sync_playwright() as p:
        ...
        while page_num <= 5 and not stop and collected_count < max_articles:
            list_url = f'{LIST_URL}?curPage={page_num}'
            print(f"\n   [PAGE] 페이지 {page_num}...")
            ...
            for i in range(row_count):
                ...
                try:
                    card = rows.nth(i)
                    link_elem = card.locator('a[href*="/nbtnewsBU/"]').first
                    ...
                    if not href or '/nbtnewsBU/' not in href: continue

                    card_text = safe_get_text(card)
                    ...
                    if date_match:
                        n_date = date_match.group(0)

                    # 제목 추출
                    lines = [l.strip() for l in card_text.split('\n') if l.strip()]
                    title = ''
                    for line in lines:
                        if re.match(r'^\d{4}-\d{2}-\d{2}$', line): continue
                        if re.match(r'^\d{3}-\d{3,4}-\d{4}$', line): continue
                        if '051-' in line or '| ' in line: continue
                        if len(line) > 10:
                            title = line
                            break

                    if not title: continue
                    title = re.sub(r'\s*새로운글\s*', '', title).strip()[:200]
