# -*- coding: utf-8 -*-
"""경상북도 보도자료 스크래퍼 v4.1 (Simple Playwright)
- 2026-01 신규 사이트 구조 대응 (inseri.gb.go.kr)
- ul.report_list 카드 구조
"""
...
REGION_NAME = '경상북도'
BASE_URL = 'https://inseri.gb.go.kr'
LIST_URL = 'https://inseri.gb.go.kr/Main/page.do?mnu_uid=6792&BD_CODE=bbs_bodo&cmd=1'
...
def normalize_date(date_str: str) -> str:
    """날짜 정규화 (YY-MM-DD 또는 YYYY-MM-DD 형식 처리)"""
    if not date_str: return datetime.now().strftime('%Y-%m-%d')
    date_str = date_str.strip().replace('.', '-').replace('/', '-')
    try:
        # YY-MM-DD 형식 (예: 26-01-07)
...
def fetch_detail(page: Page, url: str) -> Tuple[str, Optional[str], Optional[str], Optional[str]]:
    ...
    try:
        ...
        # YY-MM-DD 형식 먼저 시도
        date_match = re.search(r'(\d{2})-(\d{1,2})-(\d{1,2})', page_text)
        if date_match:
            ...
            if year < 50:
                ...
                year = 1900 + year
            pub_date = f"{year}-{int(m):02d}-{int(d):02d}"
        else:
            # YYYY-MM-DD 형식
        ...
        for sel in CONTENT_SELECTORS:
            try:
                ...
                for i in range(min(imgs.count(), 5)):
                    ...
                    if src and not any(x in src.lower() for x in ['icon', 'btn', 'logo', 'banner', 'bg', 'button', 'bodo_img.gif']):
...
def collect_articles(days: int = 3, max_articles: int = 30, start_date: str = None, end_date: str = None, headless: bool = True) -> List[Dict]:
    ...
    print(f"\n{'='*60}")
    print(f"[{REGION_NAME}] 보도자료 스크래퍼 v4.1")
    print(f"{'='*60}")
    ...
    with sync_playwright() as p:
        ...
        page_num = 0  # Start=0, 10, 20...
        stop = False
        collected_count = 0

        while page_num < 50 and not stop and collected_count < max_articles:
            start_idx = page_num * 10
            list_url = f'{LIST_URL}&Start={start_idx}'
            print(f"\n   [PAGE] 페이지 {page_num + 1} (Start={start_idx})...")
            ...
            for i in range(row_count):
                ...
                try:
                    ...
                    # 제목과 링크 추출 (subject 영역)
                    link_elem = row.locator('.subject a').first
                    if not link_elem or link_elem.count() == 0:
                        link_elem = row.locator('a').first
                    if not link_elem or link_elem.count() == 0: continue

                    # title 속성에서 제목 추출 (새글 이미지 등 방지)
                    title = safe_get_attr(link_elem, 'title')
                    if not title:
                    ...
                    # 날짜 추출 (classify 영역의 span들)
                    n_date = None
                    try:
                        classify_spans = row.locator('div.classify span')
                        if classify_spans.count() >= 3:
                            date_text = safe_get_text(classify_spans.nth(2))
                            n_date = normalize_date(date_text) if date_text else None
                    except: pass
                    ...
                    list_thumb = None
                    try:
                        thumb_img = row.locator('div.report_img img').first
                        if thumb_img and thumb_img.count() > 0:
                            thumb_src = safe_get_attr(thumb_img, 'src')
                            if thumb_src and 'bodo_img.gif' not in thumb_src:
                                list_thumb = urljoin(BASE_URL, thumb_src) if not thumb_src.startswith('http') else thumb_src
                    except: pass

                    link_data.append({'title': title, 'url': full_url, 'list_date': n_date, 'list_thumb': list_thumb})
                except: continue
            ...
            for item in new_link_data:
                ...
                # 리스트 썸네일 우선 사용
                if not thumbnail_url and item.get('list_thumb'):
                    cloudinary_url = download_and_upload_image(item['list_thumb'], BASE_URL, folder=REGION_CODE)
                    if cloudinary_url:
                        thumbnail_url = cloudinary_url
