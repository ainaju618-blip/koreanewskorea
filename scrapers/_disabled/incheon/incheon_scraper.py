# -*- coding: utf-8 -*-
"""인천광역시 보도자료 스크래퍼 v4.0 (Simple Playwright)
- 운영서버 방식으로 단순화
- 리스트 구조 (ul > li)
"""
...
LIST_ROW_SELECTORS = [
    'main ul > li:has(a[href*="/IC010205/view"])',
    'ul > li:has(a[href*="view"])',
    '.board_list li',
    'ul.list > li',
]

CONTENT_SELECTORS = [
    'div.board-view-contents',  # 인천시 본문 영역
    'div.view_content',
    'div.board_view',
    '.article_content',
    'div.bbs_view',
            ...
            for i in range(row_count):
                ...
                try:
                    ...
                    if title_elem.count() > 0:
                        ...
                        full_text = safe_get_text(link_elem).strip()
                    ...
                    n_date = None
                    try:
                        item_text = safe_get_text(row)
                        date_match = re.search(r'제공일자\s*(\d{4}[-./]\d{1,2}[-./]\d{1,2})', item_text)
                        if date_match:
                            n_date = normalize_date(date_match.group(1))
                        else:
