# -*- coding: utf-8 -*-
"""광주광역시 보도자료 스크래퍼 v4.0 (Simple Playwright)
- 운영서버 방식으로 단순화
- 별도 뉴스 포털 (news.gwangju.go.kr)
- 갤러리 형식
"""
...
REGION_CODE = 'gwangju'
REGION_NAME = '광주광역시'
BASE_URL = 'https://news.gwangju.go.kr'
LIST_URL = 'https://news.gwangju.go.kr/gallery.es?mid=a40205000000&bid=0009&cg_code=C02'
...
def collect_articles(days: int = 3, max_articles: int = 30, start_date: str = None, end_date: str = None, headless: bool = True) -> List[Dict]:
    ...
    with sync_playwright() as p:
        ...
        while page_num <= 5 and not stop and collected_count < max_articles:
            list_url = f'{LIST_URL}&nPage={page_num}'
            print(f"\n   [PAGE] 페이지 {page_num}...")
            ...
            for i in range(row_count):
                ...
                try:
                    ...
                    link_data.append({'title': title, 'url': full_url, 'list_date': None})
                except: continue
            ...
            for item in new_link_data:
                ...
                final_date = detail_date or datetime.now().strftime('%Y-%m-%d')
