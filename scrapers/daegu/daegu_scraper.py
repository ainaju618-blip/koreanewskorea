"""
대구광역시 보도자료 스크래퍼 - Enterprise Stealth v3.0
...
[특이사항]:
- 별도 뉴스룸 서브도메인 (info.daegu.go.kr)
- PHP 기반 페이지
- 페이지네이션: &bpage={page_num}
- 테이블 구조 (table tbody tr): 순번, 제목, 첨부파일, 등록일
"""
...
REGION_NAME = '대구광역시'
CATEGORY_NAME = '전국'
BASE_URL = 'https://info.daegu.go.kr'
LIST_URL = 'https://info.daegu.go.kr/newshome/mtnmain.php?mtnkey=scatelist&mkey=26'

# 대구는 bpage 파라미터 사용
PAGE_PARAM = 'bpage'

# 대구는 테이블 구조 (table tbody tr)
LIST_ROW_SELECTORS = [
    'table tbody tr',
    'table tr:not(:first-child)',  # 헤더 제외
    '.board_list table tbody tr',
]

CONTENT_SELECTORS = [
    'table tbody',           # 대구: 본문이 테이블 구조 안에 있음
    'div.view_content',
    'div.article_content',
    'div.news_content',
    'div.board_view',
        ...
        while page_num <= 5 and not stop and collected_count < max_articles:
            ...
            for i in range(row_count):
                ...
                try:
                    ...
                    # 테이블 구조: td 개수 확인 (헤더 행 제외)
                    cells = row.locator('td')
                    if cells.count() < 3: continue  # 데이터 행이 아님
                    ...
                    # 상대 경로 처리 (./mtnmain.php?...)
                    if href.startswith('./'):
                        href = href[2:]  # ./ 제거
                    full_url = href if href.startswith('http') else urljoin(LIST_URL.rsplit('/', 1)[0] + '/', href)

                    # 마지막 셀에서 날짜 추출 (YYYY.MM.DD 형식)
                    n_date = None
                    try:
                        date_cell = cells.last
                        date_text = safe_get_text(date_cell)
                        if date_text:
