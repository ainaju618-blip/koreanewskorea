# -*- coding: utf-8 -*-
"""Jeonnam Education Office (Schools) Press Release Scraper v1.0
- Site: https://www.jnedu.kr/
- Target: Schools (S1N3) - Elementary, Middle, High Schools
- Articles: ~18,683 total (935 pages)
...
# ============================================
# Constants - SCHOOLS (S1N3)
# ============================================
REGION_CODE = 'jeonnam_edu_school'
REGION_NAME = '전남교육청 학교'
CATEGORY_NAME = '전남'
BASE_URL = 'https://www.jnedu.kr'
LIST_URL = 'https://www.jnedu.kr/news/articleList.html?sc_section_code=S1N3&view_type=sm'
...
def fetch_detail(page: Page, url: str) -> Tuple[str, Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    if original_image_url:
        try:
            cloudinary_url = download_and_upload_image(original_image_url, BASE_URL, folder="jeonnam_edu_school")
    ...
    # 5. Extract school name (department)
    department = None
    try:
        byline = page.locator('header.article-view-header .infomation')
        if byline.count() > 0:
            byline_text = byline.inner_text()
            # Extract school name from byline (e.g., "XX초등학교", "XX중학교", "XX고등학교")
            match = re.search(r'(\S+(?:초등학교|중학교|고등학교|고|중|초))', byline_text)
            if match:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Jeonnam Education Office Schools (S1N3) Press Release Scraper')
