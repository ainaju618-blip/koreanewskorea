...
LIST_URL = 'https://www.hwasun.go.kr/www/news/press'
        ...
        for pn in range(1,4):
            ...
            for i in range(min(rows.count(),10)):
                try:
                    ...
                    data = {'title':title,'content':content or f"원본: {url}",'published_at':f"{datetime.now().strftime('%Y-%m-%d')}T09:00:00+09:00",'original_link':url,'source':REGION_NAME,'category':CATEGORY_NAME,'region':REGION_CODE,'thumbnail_url':thumb}
