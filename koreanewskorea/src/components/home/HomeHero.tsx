...
import { CURRENT_SITE, getAllRegionSlugs } from '@/config/site-regions';

/**
 * Korea NEWS Home Hero Section (Server Component)
 * ================================================
 * Fetches data on server for instant LCP
 * Uses CURRENT_SITE config for region-specific content
 */
...
// Default regions based on CURRENT_SITE primary regions
const DEFAULT_REGIONS = [
    ...CURRENT_SITE.regions.primary.slugs,
    ...CURRENT_SITE.regions.adjacent1.slugs.slice(0, 2),
];
...
async function getHeroData() {
    ...
    // 5. Fetch 2nd latest primary region article for sidebar
    const primaryRegionNames = CURRENT_SITE.regions.primary.names;
    const primaryRegionSlugs = CURRENT_SITE.regions.primary.slugs;
    const primaryFilters = [
        ...primaryRegionNames.map(name => `category.eq.${name}`),
        ...primaryRegionSlugs.map(slug => `region.eq.${slug}`)
    ].join(',');
    ...
    const sidebarArticle = primaryArticles && primaryArticles.length > 1
        ? primaryArticles[1]
        : (primaryArticles && primaryArticles[0]) || null;
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-4">
                                    <span className="inline-block px-2 py-0.5 bg-red-600 text-white text-xs font-bold rounded mb-2">
                                        {CURRENT_SITE.regions.primary.names[0]}
                                    </span>
