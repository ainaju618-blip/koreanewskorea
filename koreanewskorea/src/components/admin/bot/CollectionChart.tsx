...
export function CollectionChart({ dailyStats, isLoading }: CollectionChartProps) {
        ...
        return (
            <div className="bg-[#161b22] rounded-xl border border-[#30363d] shadow-sm p-6">
                <div className="animate-pulse">
                    <div className="h-4 bg-[#21262d] rounded w-1/4 mb-4"></div>
    ...
    if (!dailyStats || dailyStats.length === 0) {
        return (
            <div className="bg-[#161b22] rounded-xl border border-[#30363d] shadow-sm p-6">
                <h3 className="font-semibold text-[#e6edf3] mb-4">일별 수집 추이</h3>
    ...
    return (
        <div className="bg-[#161b22] rounded-xl border border-[#30363d] shadow-sm overflow-hidden">
            <div className="p-4 border-b border-[#21262d] bg-[#0d1117]/50 flex justify-between items-center">
                <h3 className="font-semibold text-[#e6edf3]">일별 수집 추이</h3>
                <div className="flex items-center gap-4 text-sm">
                    <span className="text-[#8b949e]">
                        총 <span className="font-bold text-[#e6edf3]">{totalArticles.toLocaleString()}</span>건
                    </span>
                    <span className="text-[#8b949e]">
                        평균 <span className="font-bold text-[#e6edf3]">{avgArticles}</span>건/일
                    </span>
                    {trendPercent !== 0 && (
                        <span className={`flex items-center gap-1 ${trendPercent > 0 ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <div className="p-4">
                ...
                <div className="flex items-end gap-1 h-32">
                    {dailyStats.map((stat, idx) => {
                        ...
                        return (
                            ...
                            >
                                {/* Tooltip */}
                                <div className="opacity-0 group-hover:opacity-100 transition-opacity mb-1 text-xs text-[#c9d1d9] whitespace-nowrap">
                                    {stat.articles}건
                                </div>
                                ...
                                {/* Date Label */}
                                <span className={`text-[10px] mt-1 ${isToday ? 'text-blue-400 font-bold' : 'text-[#6e7681]'}`}>
                ...
                {/* Legend */}
                <div className="flex items-center justify-center gap-4 mt-4 pt-3 border-t border-[#21262d] text-xs text-[#8b949e]">
