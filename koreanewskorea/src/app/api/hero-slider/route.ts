...
// GET: Fetch hero slider articles for Gwangju regional site
export async function GET(req: NextRequest) {
    try {
        // 1. Fetch gwangju-specific hero_slider settings (separate from main site)
        const { data: settingData } = await supabaseAdmin
            .from('site_settings')
            .select('value')
            .eq('key', 'hero_slider_gwangju')
            .single();
