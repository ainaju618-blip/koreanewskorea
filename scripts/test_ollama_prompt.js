/**
 * Test script for new Ollama prompt
 * Tests 5 sample articles and compares input/output lengths
 */

const articles = [
  {
    id: 1,
    title: "Short Article (191 chars)",
...
조 대표는 "입맛이 떨어지는 계절, 어르신들이 김을 밑반찬으로 건강하게 겨울을 나시길 바란다"고 전했다.`
  ...
  {
    id: 2,
    title: "Short-Medium Article (491 chars)",
...
광주서부교육지원청 이성철 교육장은 "협약을 통해 교육 예산의 한계를 극복하고 학생들에게 실질적인 도움을 줄 수 있는 상생 모델을 구축했다"며 "퇴직 교원의 풍부한 교육 경험을 바탕으로 학생 맞춤형 지도의 전문성을 더해 학생들이 배움의 즐거움을 느끼도록 하겠다"고 말했다.`
  ...
  {
    id: 3,
    title: "Medium Article (595 chars)",
...
*`
  ...
  {
    id: 4,
    title: "Medium-Long Article (774 chars)",
...
function buildPrompt(content) {
  ...
  let lengthGuidance = '';
  let minOutputLength = 0;

  if (inputLength < 500) {
    lengthGuidance = '[짧은 보도자료] 리드 문단(핵심 팩트) -> 세부 내용';
    minOutputLength = Math.floor(inputLength * 0.9);
  } else if (inputLength < 1500) {
    lengthGuidance = '[중간 보도자료] 리드 문단 -> 구조화된 정보 -> 부가 설명 -> 인용구(있으면)';
    minOutputLength = Math.floor(inputLength * 0.8);
  } else {
    lengthGuidance = '[긴 보도자료] 리드 문단 -> 소제목1 + 내용 -> 소제목2 + 내용 -> 인용구';
    minOutputLength = Math.floor(inputLength * 0.7);
  }

  return {
    prompt: `# Role
너는 광주광역시와 전라남도 지역 현안에 정통한 20년 경력의 지역 일간지 편집국장이다.
지자체 보도자료를 분석하여, 지역 주민들에게 신뢰를 주는 기사로 재구성하는 것이 네 역할이다.
...
- "획기적인", "놀라운", "역대급" 등 주관적 수식어를 배제한다.
- 사실과 수치 중심으로 서술한다.

## 4. 길이 유지 원칙 (Length Preservation) - 매우 중요!
- 원본 길이: ${inputLength}자
- 최소 출력 길이: ${minOutputLength}자 이상
...
async function testArticle(article) {
  ...
  console.log(`Expected Min Output: ${minOutputLength} chars`);
  ...
  try {
    ...
    console.log(`Length Ratio: ${ratio}%`);
    console.log(`Time: ${(elapsed/1000).toFixed(1)}s`);
    console.log(`Status: ${passed ? 'PASS' : 'FAIL'} (min: ${minOutputLength})`);
...
async function runAllTests() {
  console.log('Starting Ollama Prompt Tests...');
  console.log('Model: benedict/linkbricks-hermes3-llama3.1-8b-korean-advanced-q4');
  ...
  console.log(`${'='.repeat(60)}`);
  console.log('| # | Input | Output | Min Req | Ratio | Status |');
  console.log('|---|-------|--------|---------|-------|--------|');

  for (const r of results) {
    const status = r.passed ? 'PASS' : 'FAIL';
    console.log(`| ${r.id} | ${r.inputLength} | ${r.outputLength} | ${r.minRequired} | ${r.ratio}% | ${status} |`);
  }
