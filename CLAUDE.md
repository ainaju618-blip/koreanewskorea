...
│  ⚠️ 기본값 = 대화 모드 (DEFAULT)                             │
│                                                              │
│  명확한 실행 트리거가 없으면 → 대화 모드 유지                │
│  애매하면 → 먼저 물어본다                                    │
│                                                              │
│  "이 부분을 수정할까요, 아니면 먼저 논의할까요?"            │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                              │
│  [한국어 문장 어미 - 관찰/의견/수사적 질문]                  │
│  "~지..", "~네..", "~구나", "~잖아" → 관찰/의견 표현        │
│  "~아니야?", "~겠지?", "~인가?" → 수사적 질문 (동의 구함)   │
│  "~것 같아", "~인듯", "~아닐까" → 추측/의견                  │
│                                                              │
│  → 도구 사용 없이 대화로만 응답                             │
...
```
┌─────────────────────────────────────────────────────────────┐
│  주인님 의도가 불명확하면 → 먼저 물어본다 (MUST)            │
│                                                              │
│  예시:                                                       │
│  주인님: "이 부분 문제가 있는 것 같아"                       │
│  Claude: "네, 어떤 부분이 문제인지 말씀해주시겠어요?         │
│           아니면 제가 분석해서 수정할까요?"                  │
│                                                              │
│  주인님: "다른 AI가 이렇게 했더라고"                         │
│  Claude: "어떤 점이 잘못된 것 같으신가요?                    │
│           논의 후 제가 개선안을 적용할까요?"                 │
│                                                              │
│  ❌ 금지: 애매한 상황에서 바로 코드 수정/커밋               │
│  ✅ 필수: 대화로 의도 파악 → 명확한 지시 받기 → 실행        │
└─────────────────────────────────────────────────────────────┘
```
...
```
[BAD] 주인님: "다른 AI는 잘못이 없지.. 우리가 규칙을 잘못알려줘서 그렇지.."
      → "~지.." = 관찰/의견 표현 → 대화 모드여야 함
      → Claude가 바로 CLAUDE.md 수정 + 커밋 = P0 위반

[GOOD] 주인님: "다른 AI는 잘못이 없지.. 우리가 규칙을 잘못알려줘서 그렇지.."
       Claude: "맞습니다. 문서에 CSS 위치가 명확히 안내되지 않았네요.
                이 부분을 보강할까요?"
       주인님: "그래 보강해"  ← 명확한 실행 트리거
       Claude: (이제 수정 작업 시작)
```
