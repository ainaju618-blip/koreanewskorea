...
> **⚠️ 모든 UI/기능 변경은 반드시 로컬에서 먼저 확인한 후 배포한다.**
...
│                                                              │
│  Step 1: 코드 수정 완료                                      │
│  Step 2: 로컬 서버 실행 (npm run dev)                        │
│  Step 3: localhost:3000 에서 직접 확인                       │
│          ├── UI 렌더링 정상 여부                             │
│          ├── 데이터 표시 정상 여부                           │
│          ├── 콘솔 에러 없음 확인                             │
│          └── 기능 동작 테스트                                │
│  Step 4: 확인 완료 후 → Git 커밋/푸시                        │
│  Step 5: Vercel 자동 배포 → 프로덕션 확인                    │
│                                                              │
│  📋 확인 URL:                                                │
│     - 개발서버: http://localhost:3000                        │
│     - 프로덕션: https://www.koreanewsone.com                 │
└─────────────────────────────────────────────────────────────┘
```
...
| 문제 | 결과 |
|------|------|
| 로컬 미확인 후 배포 | 프로덕션에서 깨진 UI 노출 |
| DB 쿼리 오류 미확인 | "Preparing..." 같은 빈 화면 |
| 콘솔 에러 무시 | 사용자 경험 저하 |
...
## UI 변경 시 텍스트 미리보기 (P1 - MUST)

> **UI/텍스트 변경 완료 후, 변경된 모습을 텍스트 아스키 아트로 주인님께 보여준다.**

```
┌─────────────────────────────────────────────────────────────┐
│  📋 UI 변경 보고 형식 (MUST)                                 │
│                                                              │
│  변경 완료 후 아래처럼 텍스트로 미리보기 제공:               │
│                                                              │
│  예시 1 - 버튼/탭 변경:                                      │
│  ┌─────────────────────────────────────────────┐             │
│  │  [내지역기사]  [전체]  [내 기사]             │             │
...
| 장점 | 설명 |
|------|------|
| **즉각적 확인** | 주인님이 로컬 서버 없이도 변경 모습 파악 |
| **의사소통 명확** | "이렇게 바뀝니다" 시각적 전달 |
| **실수 방지** | 잘못된 변경 즉시 발견 가능 |
...
### 작업 완료 보고 시 확인 URL 안내 (MUST)

> **⚠️ 주인님은 개발서버 또는 프로덕션 서버 둘 다에서 확인할 수 있습니다.**

```
┌─────────────────────────────────────────────────────────────┐
│  작업 완료 후 "확인해보세요" 안내 시 (MUST)                  │
│                                                              │
│  ✅ 올바른 안내:                                             │
│     "수정 완료했습니다. 확인해보세요:                        │
│      - 개발서버: http://localhost:3000/[경로]                │
│      - 프로덕션: https://www.koreanewsone.com/[경로]"        │
│                                                              │
│  ❌ 잘못된 안내:                                             │
│     "koreanewsone.com에서 확인해보세요" (프로덕션만 안내)    │
│                                                              │
│  💡 이유:                                                    │
│     - 주인님은 배포 전 개발서버에서 먼저 확인할 수 있음      │
│     - 배포 후에는 프로덕션에서 확인할 수 있음                │
│     - 두 옵션 모두 제공해야 주인님이 선택 가능               │
└─────────────────────────────────────────────────────────────┘
```
...
## E.3 개발 도구 (Development Tools)

### 원클릭 개발서버 런처

> **위치:** `C:\Users\user\OneDrive\바탕 화면\koreanews-dev.bat`

**기능:**
- 기존 개발 서버 터미널 자동 종료
- 포트 3000 점유 프로세스 강제 종료
- 프로젝트 경로 및 Node.js 설치 확인
- Next.js 캐시 (.next) 자동 삭제
- 개발 서버 시작 + 브라우저 자동 실행

**사용법:**
```
바탕화면의 koreanews-dev.bat 더블클릭
→ 포트 3000 항상 고정
...
```
┌─────────────────────────────────────────────────────────────┐
│  ⚠️ 배치파일에서 자기 자신을 종료하지 않도록 주의!          │
│                                                              │
│  ❌ 잘못된 순서:                                             │
│     title "My App"                                           │
│     taskkill /FI "WINDOWTITLE eq My App*"  ← 자살           │
│                                                              │
│  ✅ 올바른 순서:                                             │
│     taskkill /FI "WINDOWTITLE eq My App*"  ← 먼저 종료      │
│     title "My App"                          ← 나중에 설정    │
└─────────────────────────────────────────────────────────────┘
```
