...
export default function Dec30Page() {
  ...
  const runAiProcessing = async () => {
    ...
    // If Ollama appears offline, show auto-start message
    if (ollamaStatus !== 'online') {
      addLog(`[AI] Ollama 오프라인 - 백그라운드에서 자동 시작 중...`, 'warning');
    }
    addLog(`[AI] ========================================`, 'info');
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        // Update Ollama status after successful start (backend may have started it)
        setTimeout(() => checkOllama(true), 2000);
        pollAiStatus(startTime, targetCount);
      } else {
        addLog(`[AI] 실패: ${data.error || data.message}`, 'error');
        setIsAiRunning(false);
        // Refresh Ollama status on failure
        checkOllama(false);
      }
    } catch (error: unknown) {
      const err = error as Error;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-6">
          ...
          <button
            ...
            disabled={isAiRunning || pendingCount === 0}
            className="w-full py-2 px-4 bg-[#238636] hover:bg-[#2ea043] disabled:bg-[#21262d] disabled:text-[#484f58] text-white font-medium rounded transition-colors flex items-center justify-center gap-2"
          >
            {isAiRunning ? (
              <>
