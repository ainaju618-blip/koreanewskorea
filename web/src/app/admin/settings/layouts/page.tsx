                ...
                <div className="p-4 space-y-2">
                    {layouts.filter(l => l.is_active).map((layout, idx) => {
                        ...
                        return (
                            <div
                                key={layout.id}
                                className="p-3 bg-gradient-to-r from-purple-50 to-white border border-purple-100 rounded-lg flex items-center gap-3"
                            >
                                <span className="text-xs font-bold text-purple-400 w-6">{idx + 1}</span>
                                <Icon className="w-5 h-5 text-purple-500" />
                                <span className="font-medium text-gray-800">{layout.title || layout.section_name}</span>
                                <span className="text-xs text-gray-500">({layout.items_count}개 항목)</span>
                            </div>
                        );
                    })}
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-gray-50 border-b">
                            ...
                            <th className="p-4 text-xs font-semibold text-gray-500 uppercase">섹션</th>
                            <th className="p-4 text-xs font-semibold text-gray-500 uppercase">타입</th>
                            <th className="p-4 text-xs font-semibold text-gray-500 uppercase">데이터 소스</th>
                            <th className="p-4 text-xs font-semibold text-gray-500 uppercase">항목 수</th>
                            <th className="p-4 text-xs font-semibold text-gray-500 uppercase">상태</th>
                            <th className="p-4 text-xs font-semibold text-gray-500 uppercase">순서</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {layouts.map((layout, idx) => {
                            const Icon = getSectionIcon(layout.section_type);
                            return (
                                <tr key={layout.id} className={`hover:bg-gray-50 ${!layout.is_active ? 'opacity-50' : ''}`}>
                                    <td className="p-4">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <div className="font-medium text-gray-900">{layout.section_name}</div>
                                                {layout.title && (
                                                    <div className="text-xs text-gray-500">"{layout.title}"</div>
                                                )}
                                            </div>
                                        </div>
                                    </td>
                ...
                <>
                    ...
                    <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl z-50 w-[500px] max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-4">
                            {/* 섹션 이름 */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">섹션 이름 (내부용) *</label>
                                <input
                                    type="text"
                            ...
                            {/* 표시 제목 */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">표시 제목</label>
                                <input
                                    type="text"
                            ...
                            {/* 데이터 소스 */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">데이터 소스</label>
                                <select
                            ...
                            {formData.source_type === 'category' && (
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">Ctrl+클릭으로 여러 개 선택</p>
                                </div>
                            )}

                            {/* 항목 수 */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">표시할 항목 수</label>
                                <input
                                    type="number"
                                    min={1}
                                    max={20}
                            ...
                            <label className="flex items-center gap-2">
                                ...
                                />
                                <span className="text-sm text-gray-700">"더보기" 링크 표시</span>
                            </label>

                            {/* 배경색 */}
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">배경색 (선택)</label>
                                <input
                                    type="text"
            ...
            {deleteModal.isOpen && deleteModal.layout && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 mx-4">
                        <h3 className="text-lg font-bold text-gray-900 mb-2">🗑️ 섹션 삭제</h3>
